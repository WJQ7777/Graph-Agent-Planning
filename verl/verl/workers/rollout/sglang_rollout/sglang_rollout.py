...
from verl.tools.xml_tool_parser import XMLToolParser
...
class SGLangRollout(BaseRollout):
    def __init__(
        ...
        **kwargs,
        ...
        new_template = """{%- if messages[0]['role'] == 'system' %}
                ...
                {%- if message.tool_calls %}
                    {%- for tool_call in message.tool_calls %}
                        ...
                        {%- if tool_call.arguments is mapping %}
                            {%- if tool_call.arguments.query is defined %}
                                {{- '\n<' + tool_call.name + '>' + '\n' + tool_call.arguments.query + '\n' + '</' + tool_call.name + '>\n' }}
                            {%- else %}
                                {%- set first_value = tool_call.arguments.values() | list | first %}
                                {{- '\n<' + tool_call.name + '>' + '\n' + first_value + '\n' +'</' + tool_call.name + '>\n' }}
                            {%- endif %}
                        {%- else %}
                            {{- '\n<' + tool_call.name + '>' + tool_call.arguments + '</' + tool_call.name + '>\n' }}
                        {%- endif %}
                    {%- endfor %}
                {%- endif %}
    ...
    def _init_sampling_params(self, **kwargs):
        kwargs = dict(
            ...
            custom_params={"token_ids": 151645}
        )
        ...
        if self.use_xml_tool_parser:
            print('use_xml_tool_parser open, add more stop words')
            existing_stop = kwargs.get("stop", [])
            if isinstance(existing_stop, str):
                existing_stop = [existing_stop]

            for phrase in self._stop_phrases:
                if phrase not in existing_stop:
                    existing_stop.append(phrase)
            kwargs["stop"] = existing_stop
    ...
    def _initialize_tools(self, config, tokenizer):
        ...
        use_xml_tool_parser = config.multi_turn.get("use_xml_tool_parser", False)
        self.use_xml_tool_parser=use_xml_tool_parser
        if use_xml_tool_parser:
            tool_call_parser_type = "xml"
            function_call_parser = XMLToolParser(tool_schemas)
            self._stop_phrases = function_call_parser.get_stop_phrases()
        else:
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            kwargs = dict(
                ...
                presence_penalty=self.config.val_kwargs.get("presence_penalty", 0.0),  
                frequency_penalty=self.config.val_kwargs.get("frequency_penalty", 0.0),  
                repetition_penalty=self.config.val_kwargs.get("repetition_penalty", 1.0),
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    _messages_list_of_dic = [message.model_dump() for message in _req.messages]
                    async def get_canned_web_search_response():
                        response_text = "I have performed a web search with the same query before. To find new information, I should try a more diverse or specific search query."
                        return (response_text, 0.0, {})
                    async def get_canned_crawl_page_response():
                        response_text = "I have crawled this URL before. Re-crawling the same page is unlikely to yield new information. I should look for other URLs or synthesize the information I already have."
                        return (response_text, 0.0, {})
                    ...
                    for tool_call in parsed_tool_calls:
                        ...
                        is_duplicate = False
                        if (tool_name == "web_search" or tool_name == "wiki_search") and query in _req.web_search_history:
                            tasks_to_execute.append(get_canned_web_search_response())
                            is_duplicate = True
                        elif tool_name == "crawl_page" and query in _req.crawl_page_history:
                            tasks_to_execute.append(get_canned_crawl_page_response())
                            is_duplicate = True
                    ...
                    tool_call_results = await asyncio.gather(*tasks_to_execute)
                    ...
                    for tool_call, (resp, reward, metrics) in zip(parsed_tool_calls, tool_call_results):
                        ...
                        # Track tool calls
                        tool_name = tool_call.function.name
                        if tool_name not in _req.tool_call_info:
                            _req.tool_call_info[tool_name] = 0
                        _req.tool_call_info[tool_name] += 1
                        
                        # Track detailed tool call information
                        _req.track_tool_call(
                            tool_name=tool_name,
                    ...
                    if len(_req.input_ids) >= self.config.max_model_len:
                        ...
                        _req.complete_reason = f"input_ids (len {len(_req.input_ids)}) is over long."
                        break
                    _req.state = AsyncRolloutRequestStateEnum.RUNNING
                else:
                ...
                if len(_req.get_generation_prompt_ids(self.tokenizer)) + 1 >= self.config.max_model_len:
                    ...
                    _req.complete_reason = f"prompt len ({len(_req.get_generation_prompt_ids(self.tokenizer))}) is over long."
                    break
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    _req.complete_reason = f"response overlong in this turn."
                    break
                else:
                    if self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        ...
                        try:
                            ...
                            # print("parse_non_stream : ", tool_calls)
                        ...
                        if len(parsed_tool_calls) > 0:
                            ...
                            _req.complete_reason = f"check has_tool_call is True but len(parsed_tool_calls) == {len(parsed_tool_calls)}."
                            break

                    else:
                        _req.add_assistant_message(self.tokenizer, content)
                        _req.complete_reason = f"generate stop word/id but no tool detect."
                        break
        _req.complete_reason +=  f" {output['meta_info']['finish_reason']}"
        _req.finish_reason = output['meta_info']['finish_reason']
        if current_turns >= self.config.multi_turn.max_turns:
            finish_reason_type = FinishReasonTypeEnum.STOP
            _req.complete_reason = f"current_turns({current_turns}) > max_turns({self.config.multi_turn.max_turns})"
            _req.finish_reason = {'type': "max_turn", "matched": None}
        ...
        # Finalize detailed tool metrics before request completion
        detailed_metrics = _req.finalize_tool_metrics()
    ...
    async def _handle_engine_call(self, _req: AsyncRolloutRequest, do_sample: bool, is_validate: bool, override_n: bool = True, **kwargs) -> dict:
        ...
        stop = self.sampling_params.get("stop", [])
        no_stop_trim = self.sampling_params.get("no_stop_trim", False)
        if not do_sample:
            kwargs = dict(
                n=1,
                presence_penalty=0.0,
        ...
        kwargs["stop"] = stop
        kwargs["no_stop_trim"] = no_stop_trim
    ...
    #     # 解码token IDs为文本
    #     generation_prompt_ids = _req.get_generation_prompt_ids(self.tokenizer)
    #     generation_prompt = self.tokenizer.decode(
    ...
    #         base_url="https://api.tensoropera.ai/v1"
    #     )
    ...
    #     output = {
    #         "text": [choice.message.content for choice in choices],
    #         "meta_info": {
    #             "finish_reason": {
    #                 "type": choices[0].finish_reason if choices else "stop"
    #             }
    #         },
    #         "token_ids": [
    #             self.tokenizer.encode(choice.message.content, add_special_tokens=False) 
    ...
    def _build_timeout_req(self, req: AsyncRolloutRequest, timeout_sec: int):
        dummy_resp = "TIMEOUT"
        eos = self.tokenizer.eos_token_id

        req.response_ids = [eos]
        req.response_attention_mask = [0]
        req.response_position_ids = [0]
        req.response_loss_mask = [0]

        req.input_ids += req.response_ids
        req.attention_mask += req.response_attention_mask
        req.position_ids += req.response_position_ids
        req.loss_mask += req.response_loss_mask

        req.add_assistant_message(self.tokenizer, dummy_resp)

        req.state = AsyncRolloutRequestStateEnum.COMPLETED
        req.finish_reason = {"type": "timeout", "matched": None}
        req.complete_reason = f"exceeded {timeout_sec}s"
        return req
    
    async def _async_rollout_with_timeout(self, req, do_sample, is_validate, timeout_sec=900, **kwargs):
        try:
            ...
            logger.warning(f"[ERROR] request rollout {req.request_id} timed-out after {timeout_sec}s")
            return self._build_timeout_req(req, timeout_sec)
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if self._tp_rank == 0:
            req_list = self._preprocess_prompt_to_async_rollout_requests(prompts, n=1 if is_validate else self.config.n)
            print("len of req_list : -----------------", len(req_list))
            loop = asyncio.get_event_loop()
            tasks = [self._async_rollout_with_timeout(req, do_sample, is_validate, timeout_sec=60 * 60) for req in req_list]
            output_req_list = loop.run_until_complete(asyncio.gather(*tasks))
        ...
        for req in sorted_output_req_list:
            ...
            complete_reason.append(req.complete_reason)
            finish_reason.append(req.finish_reason)
            tool_call_info.append(req.tool_call_info)
            
            # Add detailed tool metrics
            if hasattr(req, 'detailed_tool_metrics') and req.detailed_tool_metrics:
                detailed_tool_metrics_list.append(req.detailed_tool_metrics)
            else:
                # Fallback for requests without detailed metrics
                detailed_tool_metrics_list.append({
                    "total_tool_calls": sum(req.tool_call_info.values()) if req.tool_call_info else 0,
                    "tool_calls_by_type": req.tool_call_info.copy(),
                    "conversation_turns": len([msg for msg in req.messages if msg.role == "assistant"]),
                    "unique_tools_used": list(req.tool_call_info.keys()) if req.tool_call_info else [],
                    "unique_tools_count": len(req.tool_call_info) if req.tool_call_info else 0,
                    "tool_call_sequence": []
                })
        ...
        return DataProto(
            ...
            non_tensor_batch={
                ...
                "complete_reason": np.array(complete_reason),
                "finish_reason": np.array(finish_reason),
                "tool_call_info": np.array(tool_call_info),
                "detailed_tool_metrics": np.array(detailed_tool_metrics_list)
            },
        )
        ...
        for data_idx, raw_prompt in enumerate(prompts.non_tensor_batch["raw_prompt"]):
            for rollout_offset in range(n):
                ...
                req = AsyncRolloutRequest(
                    ...
                    chat_template_style=self.config.multi_turn.get("chat_template_style", "xml"), #TODO: 先测一下 xml，之后改为 default 默认值
                    complete_reason="",
                    finish_reason={},
                    web_search_history=[],
                    crawl_page_history=[],
